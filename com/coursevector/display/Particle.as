package com.coursevector.display {
	...
	import com.coursevector.util.Formulas;
		...
		*/
		...
		public function updateSettings(effect:EffectBase):void {
			...
			if(effect.xVel >= 0) {
				...
				vx = Formulas.randomRange(effect.xVelRandMin, effect.xVelRandMax);
			}
			
			if(effect.yVel >= 0) {
				vy = effect.yVel;
			} else if(effect.yVelRandMin && effect.yVelRandMax) {
				vy = Formulas.randomRange(effect.yVelRandMin, effect.yVelRandMax);
			}
			
			if(!isNaN(effect.scale)) {
				setScale(effect.scale);
			} else if(effect.scaleRandMin && effect.scaleRandMax) {
				setScale(Formulas.randomRange(effect.scaleRandMin, effect.scaleRandMax));
			}
		}
